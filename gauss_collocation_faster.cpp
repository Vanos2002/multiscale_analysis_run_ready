...
// ============================================================================
// ADAPTIVE IMPLICIT GAUSS COLLOCATION INTEGRATOR WITH ERROR ESTIMATION
// ============================================================================
...
private:
    ...
    double collocation_tolerance;
    int max_collocation_iterations;
    
    typedef std::function<SecularRHS(const BinaryState&, const PhysicalParams&, double)> RHSFunc;
    
public:
    AdaptiveRK4Integrator(double tol = 1e-4,
                          double h_min = 1e-5,
                          double h_max = 0.1,
                          long double max = 100,
                          double colloc_tol = 1e-12,
                          int colloc_iters = 40)
        : tolerance(tol),
          min_step(h_min),
          max_step(h_max),
          max_steps(max),
          collocation_tolerance(colloc_tol),
          max_collocation_iterations(colloc_iters) {}
    ...
    std::pair<BinaryState, double> adaptiveStep(
        ...
        BinaryState y1;
        if (!gaussCollocationStep(state, params, rhs_func, x, h, y1)) {
            return {state, std::numeric_limits<double>::infinity()};
        }

        BinaryState y_half;
        if (!gaussCollocationStep(state, params, rhs_func, x, h / 2.0, y_half)) {
            return {state, std::numeric_limits<double>::infinity()};
        }

        BinaryState y2;
        if (!gaussCollocationStep(y_half, params, rhs_func, x + 0.5 * h, h / 2.0, y2)) {
            return {state, std::numeric_limits<double>::infinity()};
        }
...
private:
    static BinaryState axpy(const BinaryState& state, const SecularRHS& rhs, double scale) {
        return {
            state.p + scale * rhs[0],
            state.alpha + scale * rhs[1],
            state.beta + scale * rhs[2]
        };
    }

    static double maxStageDiff(const SecularRHS& a, const SecularRHS& b) {
        return std::max({
            std::abs(a[0] - b[0]),
            std::abs(a[1] - b[1]),
            std::abs(a[2] - b[2])
    ...
    bool gaussCollocationStep(
        ...
        SecularRHS k1 = rhs_func(state, params, x + c1 * h);
        SecularRHS k2 = rhs_func(state, params, x + c2 * h);

        if (!std::isfinite(k1[0]) || !std::isfinite(k1[1]) || !std::isfinite(k1[2]) ||
            !std::isfinite(k2[0]) || !std::isfinite(k2[1]) || !std::isfinite(k2[2])) {
            return false;
        }

        bool converged = false;
        for (int iter = 0; iter < max_collocation_iterations; ++iter) {
            BinaryState stage1 = {
                state.p + h * (a11 * k1[0] + a12 * k2[0]),
                state.alpha + h * (a11 * k1[1] + a12 * k2[1]),
                state.beta + h * (a11 * k1[2] + a12 * k2[2])
            };
            BinaryState stage2 = {
                state.p + h * (a21 * k1[0] + a22 * k2[0]),
                state.alpha + h * (a21 * k1[1] + a22 * k2[1]),
                state.beta + h * (a21 * k1[2] + a22 * k2[2])
            };

            if (!isFiniteState(stage1) || !isFiniteState(stage2)) {
                return false;
            }

            SecularRHS k1_new = rhs_func(stage1, params, x + c1 * h);
            SecularRHS k2_new = rhs_func(stage2, params, x + c2 * h);

            if (!std::isfinite(k1_new[0]) || !std::isfinite(k1_new[1]) || !std::isfinite(k1_new[2]) ||
                !std::isfinite(k2_new[0]) || !std::isfinite(k2_new[1]) || !std::isfinite(k2_new[2])) {
                return false;
            }

            double diff = std::max(maxStageDiff(k1_new, k1), maxStageDiff(k2_new, k2));
            k1 = k1_new;
            k2 = k2_new;

            if (diff < collocation_tolerance) {
                converged = true;
                break;
            }
        }
        ...
        next_state = axpy(state, blended, h);

        return isFiniteState(next_state);
    }
};
...
std::vector<DeltaPhiRow> computeDeltaPhiVsEpsilonScan(
    ...
    const double qlt_max_step = 50.0;
    ...
    const double secular_max_step = 50.0;
...
int main(int argc, char** argv) {
    ...
    std::cout << "=== Adaptive Implicit Gauss Collocation Integration Comparison ===" << std::endl;
    ...
    plotDeltaPhiVsEpsilon(delta_phi_rows, "delta_phi_vs_epsilon_p50top20_fromcpp_gauss.png");

    return 0;
}


// implicit 2-stage Gauss-Legendre collocation method (inside gaussCollocationStep).
// Adaptivity is done by step-doubling: one full step of size h versus two half-steps h/2, then using their difference as an error estimate.
// That adaptive controller is RK-like in spirit, but the base method is Gauss collocation.
// it is adaptive implicit Gauss collocation (order 4).
